...
import { IoHomeOutline, IoBagOutline, IoGiftOutline, IoShareSocialOutline, IoHeartOutline } from "react-icons/io5";
...
function MenuIcon({ icon }: { icon: string }) {
  switch (icon) {
    case "home":  return <IoHomeOutline size={20} />;
    case "shop":  return <IoBagOutline size={20} />;
    case "gift":  return <IoGiftOutline size={20} />;
    case "share": return <IoShareSocialOutline size={20} />;
    case "heart": return <IoHeartOutline size={20} />;
    default:      return null;
  }
}
...
export default function GradientMenu() {
  ...
  const handleClick = useCallback((item: typeof menuItems[0]) => {
    if (item.icon === "share") {
      handleShare();
    } else if (item.href) {
    ...
    <ul className="hidden md:flex gap-4">
      {menuItems.map((item) => (
        ...
        >
          ...
          <span className="relative z-10 text-[#9CA3AF] transition-all duration-500 group-hover:scale-0 group-hover:opacity-0 flex items-center justify-center">
            <MenuIcon icon={item.icon} />
          </span>

          <span className="absolute z-10 text-white uppercase tracking-wider text-xs font-semibold whitespace-nowrap scale-0 opacity-0 transition-all duration-500 delay-100 group-hover:scale-100 group-hover:opacity-100">
            {item.title}
          </span>
        </li>
      ))}
