...
const menuItems = [
  ...
  { title: "Shop",  gradientFrom: "#56CCF2", gradientTo: "#2F80ED", href: "/shop",        icon: "shop"  },
...
function MenuIcon({ icon }: { icon: string }) {
  const cls = "text-xl";
  switch (icon) {
    case "home":  return <IoHomeOutline className={cls} />;
    case "shop":  return <IoStorefrontOutline className={cls} />;
    case "gift":  return <IoGiftOutline className={cls} />;
    case "share": return <IoShareSocialOutline className={cls} />;
    case "heart": return <IoHeartOutline className={cls} />;
    default:      return null;
  }
}
    ...
    <ul className="hidden md:flex gap-4">
      {menuItems.map((item) => (
        <li
          ...
          className="relative w-[48px] h-[48px] bg-[#1F2023] border border-[#2E2E38]
                     shadow-lg rounded-full flex items-center justify-center
                     transition-all duration-500 hover:w-[140px] hover:shadow-none
        ...
        >
          ...
          <span className="absolute top-[8px] inset-x-0 h-full rounded-full
                           bg-[linear-gradient(45deg,var(--gradient-from),var(--gradient-to))]
                           blur-[12px] opacity-0 -z-10
                           transition-all duration-500 group-hover:opacity-40" />

          <span className="relative z-10 text-[#9CA3AF]
                           transition-all duration-300
                           group-hover:scale-0 group-hover:opacity-0">
            <MenuIcon icon={item.icon} />
          </span>

          <span className="absolute z-10 text-white uppercase tracking-wider
                           text-xs font-semibold whitespace-nowrap
                           scale-0 opacity-0 transition-all duration-300 delay-100
